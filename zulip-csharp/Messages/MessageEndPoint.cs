...
namespace ZulipAPI.Messages
{
    public class MessageEndPoint : APIEndPoint
    {
        ...
        private IEnumerable<MessageBase> ConvertRecipients(IList<MessageBase> messages)
        {
            foreach (var message in messages)
            {
                var json = JsonConvert.SerializeObject(message.Recipient);
                try
                {
                    if (json.Contains("{"))
                        message.DisplayRecipient = JsonConvert.DeserializeObject<DisplayRecipient[]>(json);
                    else
                        message.DisplayRecipient = new DisplayRecipient[]
                        {
                            new DisplayRecipient() { StreamName = json.Replace("\"", "") } 
                        };
                }
                catch
                {
                    message.DisplayRecipient = new DisplayRecipient[] { new DisplayRecipient() { StreamName = json.Replace("\"", "") } };
                }
